...
# Edge QUIC CV - Azure Deployment Script (AMD64 Only)
# Works with locally built Docker image
...
# ---------------------------
print_header "Step 2: Build Docker Image Locally"
LOCAL_IMAGE="quic-yolo-server:latest"
print_info "Building Docker image locally..."
docker build -f Dockerfile.server -t $LOCAL_IMAGE .
print_success "Local Docker image built: $LOCAL_IMAGE"

# ---------------------------
# Step 3: Tag and push image to ACR
# ---------------------------
print_header "Step 3: Push Docker Image to ACR"
print_info "Logging in to ACR..."
az acr login --name $ACR_NAME
ACR_IMAGE="$ACR_NAME.azurecr.io/quic-yolo-server:latest"
docker tag $LOCAL_IMAGE $ACR_IMAGE
print_info "Pushing image to ACR: $ACR_IMAGE"
docker push $ACR_IMAGE
print_success "Image pushed to ACR"

# ---------------------------
# Step 4: Container Apps Extension
# ---------------------------
print_header "Step 4: Setup Container Apps"
az extension add --name containerapp --upgrade --only-show-errors 2>/dev/null || true
print_success "Container Apps extension ready"
...
# ---------------------------
print_header "Step 5: Creating Container Apps Environment"
if ! az containerapp env show --name $CONTAINER_APP_ENV --resource-group $RESOURCE_GROUP &> /dev/null; then
...
# ---------------------------
# Step 6: ACR Credentials
# ---------------------------
print_header "Step 6: Retrieve ACR Credentials"
ACR_USERNAME=$(az acr credential show --name $ACR_NAME --query username -o tsv)
ACR_PASSWORD=$(az acr credential show --name $ACR_NAME --query passwords[0].value -o tsv)
print_success "ACR credentials retrieved"
...
# ---------------------------
print_header "Step 7: Deploy Container App"
if az containerapp show --name $CONTAINER_APP_NAME --resource-group $RESOURCE_GROUP &> /dev/null; then
...
# ---------------------------
# Step 8: Application URL
# ---------------------------
print_header "Deployment Complete!"
APP_URL=$(az containerapp show --name $CONTAINER_APP_NAME --resource-group $RESOURCE_GROUP --query properties.configuration.ingress.fqdn -o tsv)
print_success "App deployed: https://$APP_URL"
echo "WebSocket URL: wss://$APP_URL"
echo "UDP Receiver: $APP_URL:5005"

# Save deployment info
cat > deployment-info.txt << EOF
Edge QUIC CV - Deployment Info
Generated: $(date)
Container App: $CONTAINER_APP_NAME
URL: https://$APP_URL
EOF
